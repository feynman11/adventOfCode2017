...
public class day4 {
	...
	public void go() {
		System.out.println("Welcolme to Advent of Code - Day 4: High-Entropy Passphrases");
		String input = getInput();
		String[] phrases = input.split("\\n");
		// Calculate closes square number to input
		double part1 = getCorrectPassphrases(phrases, false);
		double part2 = getCorrectPassphrases(phrases, true);

		System.out.println("Number of correct phrases are " + part1);
		System.out.println("Number of correct phrases (anagrams) are " + part2);

	}

	private double getCorrectPassphrases(String[] input, boolean anagrams) {
		int passPhrases = 0;
		...
		for (int i = 0; i < input.length; i++) {
			...
			for(String w : words) {
				sortedWords[j] = sortString(w);
				j++;
			}
			if(!duplicates(anagrams ? sortedWords : words)) {
				passPhrases++;
			}
		}
		return passPhrases;
	...
	boolean duplicates(final String[] array) {
